...
class ColumnParallelLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, tp_size: int, tp_rank: int):
        ...
        # local weights, only 1/tp_size of the output features
...
class RowParallelLinear(nn.Module):
    ...
    def __init__(
        ...
        # local weights, only 1/tp_size of the input features
    ...
    def forward(self, x_local: torch.Tensor) -> torch.Tensor:
        ...
        # sum across all GPUs to get the full output
...
def verify_row_parallel(rank: int, world_size: int) -> None:
    ...
    # split input into local parts for row parallel (each rank gets a slice)
    x_chunks = x_full.chunk(world_size, dim=1) # split cols
    ...
    # Forward pass with row-parallel (including all_reduce)
    ...
    if rank == 0:
        ...
        full_weight = torch.cat(gathered_weights, dim=0) # why dim 0? because we are concatenating rows
...
""")
    ...
    batch_size = 4
    hidden_size = 8
    intermediate_size = 32 # 4*hidden_size
