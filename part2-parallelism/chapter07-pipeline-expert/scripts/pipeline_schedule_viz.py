...
Bubble fraction: 27% (3 slots idle per GPU out of 11) (P = 4, M = 8, (P-1)/(M + P - 1) = 3/11 ~ 0.27)
...
Bubble fraction: 19% ((P - 1)/ (2M) = (4 - 1) / (2 * 8) = 3/16 ~ 0.19)
2M why? 1 forward and 1 backward pass step per microbatch
...
def visualize_naive_schedule(stages: int, microbatches: int) -> Tuple[str, float]:
    ...
    total_forward_time = microbatches + stages - 1 #(M + P - 1)
    total_backward_time = microbatches + stages - 1 #(M + P - 1)
    ...
    work_per_stage = 2 * microbatches # F + B per stage
    bubble_per_stage = total_time - work_per_stage # 2(M + P - 1) - 2M = 2P - 2
...
def visualize_1f1b_schedule(stages: int, microbatches: int) -> Tuple[str, float, int]:
    ...
    work_units = 2 * microbatches # F + B per stage
