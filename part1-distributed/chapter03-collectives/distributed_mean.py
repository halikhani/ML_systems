...
def compute_mean_simple(rank: int, world_size: int, data_size: int, device: torch.device) -> torch.Tensor:
    """
    ...
    local_data = torch.zeros(data_size, device=device) + (rank + 1)

    # step 1: sum across ranks
    total = local_data.clone()
    dist.all_reduce(total, op=dist.ReduceOp.SUM)
    # step 2: divide by world_size
    mean = total / world_size
    return mean, local_data


def compute_mean_weighted(rank: int, world_size: int, local_sizes: list, device: torch.device) -> torch.Tensor:
    """
    ...
    local_data = torch.zeros(local_size, device=device) + (rank + 1)
    ...
    # now local_sum contains the sum of all data across all ranks

    # Step 3: all_reduce the count
    local_count = torch.tensor([local_size], device=device)
    dist.all_reduce(local_count, op=dist.ReduceOp.SUM)
    # now local_count contains the total number of data across all ranks
