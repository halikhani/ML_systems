...
def demo_broadcast(rank: int, world_size: int, device: torch.device) -> None:
    ...
    if rank == 0:
        print("\n[Explanation] Rank 0's data [1, 2, 3, 4] was copied to all ranks.")
...
def demo_scatter(rank: int, world_size: int, device: torch.device) -> None:
    ...
    if rank == 0:
        scatter_list = [
            torch.tensor([i * 10.0, i * 100.0], device=device)
    ...
    if rank == 0:
        ...
        print("             Rank 0 got [0,1], Rank 1 got [10,100], etc.")
...
def demo_reduce(rank: int, world_size: int, device: torch.device) -> None:
    ...
    # Each rank has unique data
    tensor = torch.tensor([rank * 10.0, rank * 100.0], device=device)
    print_state(rank, world_size, "REDUCE (dst=0)", tensor, is_before=True)
    
    # reduce from all processes to rank 0
    # NOTE: reduce is an in-place operation
    dist.reduce(tensor, dst=0, op=dist.ReduceOp.SUM)
    print_state(rank, world_size, "REDUCE (dst=0)", tensor, is_before=False)
    if rank == 0:
        print("\n[Explanation] Rank 0 has the sum of all data: [0.0, 0.0] + [10.0, 100.0] + [20.0, 200.0] + [30.0, 300.0] = [60.0, 600.0].")
...
def demo_all_reduce(rank: int, world_size: int, device: torch.device) -> None:
    ...
    print_state(rank, world_size, "REDUCE (dst=0)", tensor, is_before=False)
    if rank == 0:
        print("\n[Explanation] Every rank now has the sum of all data: [0.0, 0.0] + [10.0, 100.0] + [20.0, 200.0] + [30.0, 300.0] = [60.0, 600.0].")
